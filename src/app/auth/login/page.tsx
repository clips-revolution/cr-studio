'use client'

import { useState } from 'react'
import { signIn } from 'next-auth/react'
import { useRouter } from 'next/navigation'
import { toast } from 'sonner'
import Image from 'next/image'
import BackgroundGrid from '@/components/layout/BackgroundGrid'

export default function LoginPage() {
  const router = useRouter()
  const [email, setEmail] = useState('')
  const [password, setPassword] = useState('')
  const [loading, setLoading] = useState(false)

  async function handleEmailLogin(e: React.FormEvent) {
    e.preventDefault()
    setLoading(true)
    const result = await signIn('credentials', { email, password, redirect: false })
    setLoading(false)
    if (result?.ok) {
      router.push('/')
    } else {
      toast.error('אימייל או סיסמה שגויים')
    }
  }

  async function handleGoogleLogin() {
    setLoading(true)
    await signIn('google', { callbackUrl: '/' })
  }

  return (
    <div className="relative min-h-screen bg-black overflow-hidden flex items-center justify-center">

      {/* Static background — ללא תחלופה */}
      <BackgroundGrid animate={false} />

      {/* Overlays */}
      <div className="absolute inset-0 bg-gradient-to-b from-black/85 via-black/60 to-black/90 pointer-events-none" />
      <div className="absolute inset-0 bg-gradient-to-r from-black/70 via-transparent to-black/70 pointer-events-none" />

      {/* Login card */}
      <div className="relative z-10 w-full max-w-sm px-6 fade-up">
        <div className="text-center mb-10">
          <h2 className="text-white text-3xl mb-4 tracking-widest text-center" style={{ fontFamily: 'var(--font-display)' }}>
            c<span className="text-purple-400">R</span> Studio
          </h2>
          <Image src="/logo.png" alt="cR Studio" width={150} height={45} className="mx-auto mb-3" priority />
          <p className="text-white/35 text-sm">הפכו רעיונות למציאות</p>
        </div>

        <div className="bg-white/[0.05] backdrop-blur-xl border border-white/10 rounded-2xl p-8 space-y-5">
          <button
            onClick={handleGoogleLogin}
            disabled={loading}
            className="w-full flex items-center justify-center gap-3 bg-white text-gray-900 font-medium py-3 px-4 rounded-xl hover:bg-white/90 transition-colors disabled:opacity-50 text-sm"
          >
            <svg className="w-4 h-4 ltr" viewBox="0 0 24 24">
              <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
              <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
              <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
              <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
            </svg>
            המשך עם Google
          </button>

          <div className="relative flex items-center gap-3">
            <div className="flex-1 h-px bg-white/8" />
            <span className="text-white/20 text-xs">או</span>
            <div className="flex-1 h-px bg-white/8" />
          </div>

          <form onSubmit={handleEmailLogin} className="space-y-3">
            <input
              type="email"
              value={email}
              onChange={e => setEmail(e.target.value)}
              className="w-full bg-white/5 border border-white/8 rounded-xl px-4 py-3 text-white placeholder:text-white/20 focus:outline-none focus:border-white/20 text-sm ltr"
              placeholder="your@email.com"
              required
            />
            <input
              type="password"
              value={password}
              onChange={e => setPassword(e.target.value)}
              className="w-full bg-white/5 border border-white/8 rounded-xl px-4 py-3 text-white placeholder:text-white/20 focus:outline-none focus:border-white/20 text-sm"
              placeholder="סיסמה"
              required
            />
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-white/8 hover:bg-white/14 text-white font-medium py-3 px-4 rounded-xl transition-colors disabled:opacity-50 text-sm border border-white/10"
            >
              {loading ? 'מתחבר...' : 'כניסה'}
            </button>
          </form>
        </div>
      </div>
    </div>
  )
}
